...
RtmpGuesterImpl::RtmpGuesterImpl(RTMPGuesterEvent&callback)
	: callback_(callback)
	, worker_thread_(webrtc::AnyRtmpCore::Create())
	...
	, m_pFrameReciver()
{
	...
	av_rtmp_player_->SetVideoRender(this);
...
void RtmpGuesterImpl::StartRtmpPlay(const char* url, void* render)
{
	if (!av_rtmp_started_) {
		...
		//video_render_ = webrtc::VideoRenderer::Create(render, 16, 9);
		//av_rtmp_player_->SetVideoRender(video_render_);
		av_rtmp_player_->StartPlay(url);
		worker_thread_->StartAudioTrack(this);
	}
}
...
void RtmpGuesterImpl::SetFrameReciver(FrameReciver * recv)
{
	if (recv)
	{
		m_pFrameReciver = recv;
	}
}

void RtmpGuesterImpl::OnFrame(const cricket::VideoFrame & frame)
{
	if (m_pFrameReciver)
	{
		FrameRef rFrame;
		rFrame.width = frame.width();
		rFrame.height = frame.height();
		rFrame.yData = (unsigned char *)frame.video_frame_buffer()->DataY();
		rFrame.uData = (unsigned char *)frame.video_frame_buffer()->DataU();
		rFrame.vData = (unsigned char *)frame.video_frame_buffer()->DataV();
		rFrame.yStride = frame.video_frame_buffer()->StrideY();
		rFrame.uStride = frame.video_frame_buffer()->StrideU();
		rFrame.vStride = frame.video_frame_buffer()->StrideV();

		m_pFrameReciver->OnFrame(rFrame);
	}
}
